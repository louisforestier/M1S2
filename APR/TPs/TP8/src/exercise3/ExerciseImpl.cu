...
void ExerciseImpl::run(const bool verbose) {    
	...
	OPP::CUDA::DeviceBuffer<unsigned> dev_Histogram(reference->getHistogram().data(), 256);
	OPP::CUDA::DeviceBuffer<unsigned> dev_Repartition(256);
	
    if( verbose )
        std::cout << " - compute the repartition function " << std::endl;
    execute_and_display_GPU_time(verbose, [&]() {
        auto impl = reinterpret_cast<StudentWorkImpl*>(student);
        impl->run_Repartition(dev_Histogram, dev_Repartition);
    }, nbTry);

	dev_Repartition.copyToHost(Repartition.data());	
}


bool ExerciseImpl::check() 
{
    if( !check_Repartition() ) return false;
    return true;
}
...
bool ExerciseImpl::check_Repartition()
{
    for(int i=0; i<64; ++i) std::cout << Repartition[i] << " ";
    std::cout << std::endl;
    for(int i=0; i<64; ++i) std::cout << reference->getRepartition()[i] << " ";
    std::cout << std::endl;
    if( Repartition != reference->getRepartition() ) {
