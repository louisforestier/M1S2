...
namespace OPP
{
    namespace CUDA 
    {
        ...
        public:
            ...
            DeviceBuffer(const DeviceBuffer& dv) : nbElements(dv.nbElements), sizeInBytes(dv.sizeInBytes) 
            {
                cudaMalloc(&buffer, sizeInBytes);
                getLastCudaError("Unable to allocate a DeviceBuffer ...");
                cudaMemcpy(buffer, dv.buffer, sizeInBytes, cudaMemcpyDeviceToDevice);
