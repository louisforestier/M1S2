...
{
    struct s_block_info {
        int m_world_size; // number of processus in the MPI world
        int m_rank; // rank of the current processus
    ...
    {
        s_block_info blk;
        MPI_Comm_size(MPI_COMM_WORLD, &blk.m_world_size);
        MPI_Comm_rank(MPI_COMM_WORLD, &blk.m_rank);
        blk.m_number_of_rows = (m + blk.m_world_size-1) / blk.m_world_size;
        blk.m_first_row = blk.m_number_of_rows * blk.m_rank;
...
public:
    ...
    DistributedRowMatrix(const int m, const int n, const char*fileName)
