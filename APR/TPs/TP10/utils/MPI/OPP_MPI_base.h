...
namespace OPP {
    ...
    namespace MPI {
        ...
        class Communicator {
            ...
            static int getRank(const MPI_Comm& communicator) {
                int rank;
                checkMPIErrors( MPI_Comm_rank(communicator, &rank) );
                return rank;
            }

            static int getSize(const MPI_Comm& communicator) {
                int size;
        ...
        public:
             ...
             */
            Communicator(const MPI_Comm& communicator) : 
                communicator(communicator), rank(getRank(communicator)), size(getSize(communicator))
            {
            }
