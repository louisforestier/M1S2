...
class DistributedBlockVector
{
    ...
    static s_block_info get_block_info(const OPP::MPI::Communicator&communicator, int N)
    {
        s_block_info blk;

        blk.m_size = (N + communicator.size - 1) / communicator.size;
        blk.m_offset = blk.m_size * communicator.rank;
  ...
  public:
    ...
    DistributedBlockVector(const OPP::MPI::Communicator& communicator, const int N, const char *fileName) : 
        communicator(communicator), m_block(get_block_info(communicator, N)), m_N(N)
    {
        MPI_Alloc_mem(
