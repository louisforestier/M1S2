...
void ExerciseImpl::usage( const char*const prg ) {
    ...
    std::cout << "Usage: " << (last_slash==nullptr ? prg : last_slash+1) 
        << " [ -t=threads ] [ -s=size ]" << std::endl 
        << "where -t specifies the number of threads (default is "<< number_of_threads <<")"<<std::endl
        <<"       -s  specifies the number of AVX registers (default is "<< number_of_avx_registers <<")."
        << std::endl;
}
...
void ExerciseImpl::run(const bool verbose) {    
    ...
    execute_and_display_time(verbose, [&]() {
        reinterpret_cast<StudentWorkImpl*>(student)->run(number_of_threads, input_floats, student_floats, number_of_avx_registers << 3);
    });
    if( verbose )
        std::cout << std::endl << "Test the student work with "<<(number_of_avx_registers)<<" AVX floats ..." << std::endl;
    execute_and_display_time(verbose, [&]() {
        reinterpret_cast<StudentWorkImpl*>(student)->run(number_of_threads, input_m256s, student_m256s, number_of_avx_registers);
    });
}
