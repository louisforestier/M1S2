...
void ExerciseImpl::run(const bool verbose) {    
    ...
    execute_and_display_time(verbose, [&]() {
        student_float = reinterpret_cast<StudentWorkImpl*>(student)->run(input_floats, number_of_avx_registers << 3);
    });
    if( verbose )
        std::cout << std::endl << "Test the student work with "<<(number_of_avx_registers)<<" AVX floats ..." << std::endl;
    execute_and_display_time(verbose, [&]() {
        student_m256 = reinterpret_cast<StudentWorkImpl*>(student)->run(input_m256s, number_of_avx_registers);
    });
}


bool ExerciseImpl::check() {
    float truth = float(number_of_avx_registers<<3);
    std::cout << "  awaited result: " << truth << std::endl;;
    std::cout << "   linear result: " << student_float << std::endl;
    std::cout << "vectorial result: " << student_m256 << std::endl;
    if( truth != student_float )
        return false;
    if( truth != student_m256 )
        return false;
    return true;
}
